...
femData = read.table(file="for_WGCNA_gene.xls",sep="\t",header=T);
...
datExpr = datExpr[,order(apply(datExpr,2,mad), decreasing = T)[1:10000]]
...
###load content###
traitData = read.table(file="../Clinic",sep="\t",header = T);
dim(traitData)
names(traitData)
...
library(readr)
annot <- read_delim("../GPL4133_annot.xls", 
...
table.display$overlapGenes = shortenStrings(table.display$overlapGenes, maxLength = 7000, split = "|");
head(table.display)
write.table(GOenrichment$enrichmentTable, file = "GOenrichment-enrichmentTable.xls",sep="\t",quote = F ,
          row.names = FALSE)

#The function runs for awhile and returns a long list, the most interesting component of which is
tab = GOenr$bestPTerms[[4]]$enrichment
names(tab)
write.table(tab, file = "GOEnrichmentTable.xls", sep = ",", quote = TRUE, row.names = FALSE)

tab = GOenr$bestPTerms$BP$forModule[[1]][[10]]

head(tab)
names(tab)
write.table(tab, file = "GOEnrichmentTable3.csv", sep = ",", quote = TRUE, row.names = FALSE)
...
= 90)
#plotEigengeneNetworks(MET[-15,-15], "", marDendro = c(0,4,1,2), marHeatmap = c(3,4,1,2), cex.lab = 0.8, xLabelsAngle
                      #= 90)
# Plot the dendrogram
...
#annot = read.table(file = "../anno_data.xls",sep="\t",header = F);
annot <- read_delim("~/Data/Hanlongchou/CH1712009/anno_data.xls", 
                         "\t", escape_double = FALSE, trim_ws = TRUE)
