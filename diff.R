...
nor_data=read.table("RawDataNOCtrl.txt",sep="\t",header = T,stringsAsFactors = F)
...
a=read.table("RawDataNOCtrl.txt",sep="\t",header = F,stringsAsFactors = F)[1,]
SaGo=t(a[-1:-7])
...
write.table(final,"All_compare_gene.xls",sep="\t",quote = F ,row.names = F)

#write.csv(final)

  write.table(For_ana,"for_WGCNA_gene.xls",sep="\t",quote = F ,row.names = F)
